...
export default function Acabedit() {
  ...
  const [formData, setFormData] = useState({
    name: '', model: '', type: '', ratePerKm: '', plateNumber: '', driverName: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/cars/edit/${id}`, data, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert('Car updated successfully');
      navigate('/acabs');
    } catch (err) {
      alert(err.response?.data?.error || 'Failed to update car');
    }
  };
    ...
    <div style={{ minHeight: '100vh', backgroundColor: 'transparent' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: '40px', paddingBottom: '40px' }}>
        <div style={{ backgroundColor: 'var(--color-surface)', padding: '40px', borderRadius: '10px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', width: '500px' }}>
          <h2 style={{ textAlign: 'center' }}>Edit Cab</h2>
          <form onSubmit={handleSubmit}>
            ...
            <p style={{ margin: '5px 0' }}>Update Image (optional):</p>
            <input type="file" onChange={(e) => setImage(e.target.files[0])} style={{ width: '100%', padding: '10px', margin: '0 0 10px', border: '1px solid #ccc', borderRadius: '4px' }} />
            
            <button type="submit" style={{ width: '100%', padding: '12px', backgroundColor: 'var(--color-primary)', color: 'var(--color-text)', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', marginTop: '10px' }}>Update Cab</button>
          </form>
        </div>
      </div>
    </div>
